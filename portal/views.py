...
def view_resume(request, application_id):
    ...
    """
    try:
        # Get the current supplier to verify authorization
        try:
            supplier = get_supplier_for_user_or_raise(request)
        except PermissionDenied:
            raise PermissionDenied("Only suppliers can access this page.")
        ...
        # Verify supplier owns this job/internship
        if job_app:
            job = job_app.job
            if job.supplier_id != supplier.id:
                raise PermissionDenied("You don't have access to this application.")
        else:
            internship = internship_app.internship
            if internship.supplier_id != supplier.id:
                raise PermissionDenied("You don't have access to this application.")
